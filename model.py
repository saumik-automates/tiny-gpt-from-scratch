...
def layernorm_backward_full(dy, cache):
    ...
    result = {}
    result["dbeta"] = np.sum(dy, axis=0)
    x_hat = cache["x_hat"]
    result["dgamma"] = np.sum(dy*x_hat, axis=0)
...
def token_embedding_forward(token_ids, embedding_matrix):
    ...
    return embedding_matrix[token_ids], {"token_ids": token_ids, "vocab_size": vocab_size}
...
def masked_softmax_backward(d_attn, cache):
    ...
    """
    sum_attn_d_attn = np.sum(cache["attn"]*d_attn, axis=-1, keepdims=True)
    d_scores = cache["attn"]*(d_attn - sum_attn_d_attn)
    d_masked_scores = np.where(cache["causal_mask"], d_scores, 0.0)
...
def qkv_projection_backward(d_q, d_k, d_v, cache):
    B, T, d_in = cache["x"].shape

    x_flat = x.reshape(B*T, -1)
    d_k_flat = d_k.reshape(B*T, -1)
    d_q_flat = d_q.reshape(B*T, -1)
    d_v_flat = d_v.reshape(B*T, -1)

    dw_q = x_flat.T @ d_q_flat
    dw_k = x_flat.T @ d_k_flat
    dw_v = x_flat.T @ d_v_flat
    dx_flat = d_q_flat @ cache["w_q"].T + d_k_flat @ cache["w_k"].T + d_v_flat @ cache["w_v"].T
...
def transpose_heads_to_front(x_heads):
    ...
    return np.transpose(x_heads, axes=(0, 2, 1, 3))
...
def compute_d_head(d_model, n_heads):
    if d_model % n_heads != 0:
        raise ValueError(f"n_heads = {n_heads} does not evenly divide d_model = {d_model}")
